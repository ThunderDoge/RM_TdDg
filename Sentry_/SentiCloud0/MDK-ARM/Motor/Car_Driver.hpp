...
{
	str_Motor_t(uint16_t a,uint8_t b, uint16_t max):max_mechanical_position(a),Reduction_ratio(b),max_control_value(max){}
...
{
	public:
		int16_t CurrentEncoder;			//编码器反馈转矩电流
		int16_t LastCurrentEncoder;	//编码器反馈转矩电流
...
class cloud : public manager//!扩展:云台类(6623)
{
	public:
		Motor_t *MotorType;        //!<电机(电调)参数
		float RealAngle;           //!<根据机械角计算出的真实角度
		float TargetAngle;			//!<以机械角计算出的真实角度坐标系上的目标角度值	Edit by ThunderDoge
		float RelativeAngle;		//!<相对角度。=Gyro_RealAngle - RealAngle。具体物理意义不确定，仅仅保证，底盘固定时，云台向上抬，此值增大，即可。
		//电流环
