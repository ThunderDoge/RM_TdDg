...
<<<<<<< RM2019和RM2020分界线
	...
	*		V7.21 对pid::run计算函数做了一次兼容性更新，适应没有做积分分离的参数
	*		V7.22 使用机械角模式调节Yaw轴的时候发现了无法克服8192到0的突变，疯狂转头，根据步兵上的代码对softcloud::Angle_Set，softcloud::Position_Run
						两个函数进行了一次修改，已经正常
	*		V7.3	根据RM19步兵代码移植了牵引力控制程序
...
WEAK float pid::pid_run(float err)
{
	...
	if(T==0) Iout += I	*	CurrentError;	//兼容性改动，积分时间设置为0就认为不做积分分离，一直积分
	else																//设置了积分时间则开始积分分离过程
	{
		if(HAL_GetTick() - start_time >= T)//如果达到了时间区间的话则进行积分输出
		{
			if(APP_MATH_ABS(CurrentError) < I_Limited)//仅在小于误差区间时进行I积分
				Iout += I	*	CurrentError;
			else 
				Iout=0;
			start_time = HAL_GetTick();//重新定义积分开始时间
		}
	}
	...
	Iout = app_math_Limit(Iout,IMax,-IMax);	//积分限幅
	
	PIDout = Pout + Iout + Dout;   //PID输出计算
	PIDout = app_math_Limit(PIDout,PIDMax,-PIDMax); //输出限幅
...
{
	if(APP_MATH_ABS(Current)>this->block_Current_Limit)//电流超过阈值被判定为堵转
	{
		...
		{
			if(APP_MATH_ABS(block_Angle-RealAngle) < block_err_num)//登记堵转时刻至此刻间仍未转过阈值角度
			{
...
WEAK void cloud::update(uint8_t Data[])
{
	...
	RelativeAngle = *Gyro_RealAngle - RealAngle;
}
...
WEAK void cloud::Position_Run(void)
{
	...
	float err = TargetAngle - RealAngle;	//Edit by ThunderDoge
...
WEAK void chassis::Handle()
{
	...
	for(i=0; i<4; i++)//按照四个轮子的功率计算需要受限的功率
		calcPower += 24.0f*APP_MATH_ABS(point->Motor[i]->TargetCurrent)/819.2f;
