...
#define MPU_RA_ACCEL_CONFIG_1   0x1C
#define MPU_RA_ACCEL_CONFIG_2   0x1D
#define MPU_RA_LP_CONFIG        0x1E
...
uint8_t bsp_spi_ReadReg(uint8_t regAddr);//提供给姿态解算的读数据函数
void bsp_spi_ReadRegs(uint8_t regAddr,uint8_t *pBuff,uint8_t length);//标准SPI读函数
HAL_StatusTypeDef bsp_spi_Icm20602Init(void);//ICM20602初始化

#ifdef USE_MAG
extern uint8_t bsp_spi_MagAsa[3];//读取磁力计ASA校正寄存器
void bsp_spi_MagTrig(void);//磁力计触发
uint8_t bsp_spi_MagReads(uint8_t regAddr,uint8_t *pBuff,uint8_t length);//磁力计读取
HAL_StatusTypeDef bsp_spi_Ak8975Init(void);//磁力计初始化
#endif
#endif
