...
{
    str_Motor_t(){}; //避免创建空�?�象不成�??
    str_Motor_t(uint16_t a, uint8_t b) : max_mechanical_position(a), Reduction_ratio(b){};
    uint16_t max_mechanical_position = 8192; //!<机�?��?�最大�?,默�??0x2000
    uint8_t Reduction_ratio = 19;            //!<减速比,默�??19:1
} Motor_t;                                   //!<电机(电调)参数结构体，默�?�值为3508电机及其配�?�C620电调

//PID�??以算pid输出 �??以配置pid
class pid
{
public:
    float *Custom_Diff = NULL; //!<�??定义的�?�界�??点型�??分数�?? 常用于路程环的微分环�??(即速度�??)
    uint16_t I_Time;           //!<pid时间参数 �??ms为单�?? plus专属,�??分时�??
    uint16_t D_Time;           //!<�??分时间，ms为单�??
    uint16_t I_Limited;        //!<当�??�??小于I_Limited时才进�?�I输出 plus专属
...
//private:
    ...
    float Iout;
    float Dout;
    float Dout_Accumulative; //因为有微分时间重定义,因�?�加入一�??变量真�?�用于输出D,源Dout用作�??�??�??�??
    float PIDout;
    float CurrentError;
    float LastError;
...
/** 
	* @brief 堵转检测类 
	* @details 要求在初始化的时候给定需要�?�测堵�?的电流引用和位置引用 \n
	*		  一�??不会单独使用该类，电机�?�果�??持堵�??检测功能的话会在其内部有一�??堵转检测的指针 \n
	*		  想�?�启用电机的堵转检测功能的话使�?�??�应电机类内提供的初始化函数 \n
...
{
public:
    void Block_Init(uint16_t Limit, uint16_t time, float err_num);                                 //!<进�?�堵�??参数初�?�化
...
private:
    ...
    uint32_t block_Time_Limit;    //!<堵转检测时允�?�的堵转时间
    float block_err_num;          //!<堵转时�?�测允�?�的容忍�??�??
    uint8_t block_flag = 0;       //!<处于检测堵�??的标志，不堵�??�??动恢复为0
};

class manager //电机管�?�抽象类
{
public:
    //!< 兼�?�性改�??，在线列表的数据类型�??改到int16_t
    //!<CAN1在线的ID列表兼�?�性更新，适配GM6020的ID会超�??0x208，现在[0:10]分别�??0x201�??0x20B电机在线状�?
    static int16_t CAN1_OnlineID; //!<CAN1在线的ID列表，[0:7]分别�??0x201�??0x207的电机在线状�??(此�?�是旧的说明)
    //!<CAN2在线的ID列表兼�?�性更新，适配GM6020的ID会超�??0x208，现在[0:10]分别�??0x201�??0x20B电机在线状�?
    static int16_t CAN2_OnlineID; //!<CAN2在线的ID列表，[0:7]分别�??0x201�??0x207的电机在线状�??(此�?�是旧的说明)
    RunState_t RunState = Stop;   //!<电机的运行状�??
    //!电机�??否是协同工作的，如果属于某机构的一部分，则该值可以置1，�?�时Handle()函数不会由CANSend执�?�，需要自己实现，从而完成电机组合逻辑
    uint8_t cooperative;
    uint8_t Is_Offline(void); //!<判断当前电机�??否�?�于离线状�?
    //!全局的CAN收发函数,必须必须必须先调用CANSelect()且必须在CAN接收打开前调用本函数�??认接收句�?? Update函数放在CAN接收回调里面,Send保证周期执�??,
    static void CANSelect(CAN_HandleTypeDef *canhandle1, CAN_HandleTypeDef *canhandle2);
    static void CANUpdate(CAN_HandleTypeDef *_hcan, CAN_RxHeaderTypeDef *RxHead, uint8_t *Data); //!<全局的接收�?�理函数,统一管配所有电�??
    static void UserProcess(void);                                                               //!<在PID跑完 发送前进�?�的数据处理函数，建�??以重写的形式进�?�新的�?�理
    static uint8_t CANSend(void);                                                                //!<全局的发送函数，统一管配所有电�??
    void Speed_F_Set(float f);                                                                   //!<设定前�?�量 输入电机的新的�?
    virtual void Safe_Set(void) = 0;                                                             //!<子类必须实现的纯虚函�??
protected:
    uint8_t can_code;                     //!<CAN�??,以十进制存储,百位数显示是can几，后面两位数表示id�??-1，范�??0~10
    static CAN_HandleTypeDef *CanHandle1; //!<CAN设�??1的指�??
    static CAN_HandleTypeDef *CanHandle2; //!<CAN设�??2的指�??
    ...
    uint32_t LastUpdateTime;              //!<上�?�更新的时间
    float Speed_LPF;                      //!<速度前�?�低通滤波器
    float Speed_F;                        //!<速度前�?�参�??

    virtual void update(uint8_t Data[]) = 0; //!<子类必须实现的纯虚函�??
    virtual void Handle(void) = 0;           //!<子类必须实现的发送�?�理函数
};
...
public:
    ...
    float RealAngle;          //!<根据机�?��?��?�算出的真实角度
    int16_t TargetCurrent;    //!<�??标电流�?
    ...
    block_type *block = NULL; //!<堵转对象指针，在使用堵转检测时会生成�?�象并储存指针在这里
    motor(void){};            //!<仅用于默认构造函�??
    ...
    void Speed_Set(int16_t);                                                      //!<设定速度，其实可以直接�?�定TargetSpeed
    void Angle_Set(float);                                                        //!<设定位置，其实可以直接�?�定TargetPosition
    virtual int8_t Enable_Block(uint16_t Limit, uint16_t time, uint16_t err_num); //!<到时候写�?? 注意负数 �??坑了
    virtual void Safe_Set(void);                                                  //!<设定电机进入安全模式，即发送电流�?0
protected:
    class pid *PID_In;    //!<PID内环
    class pid *PID_Out;   //!<PID外环
    int16_t LastPosition; //!<上�?�位�??
    int16_t LastSpeed;    //!<上�?�速度
...
/** 
    * @brief �??�??程电�?? \n
		* 重�?�特�??:把电机面对自�?? 逆时针转动的时�? �??程�?�数增加 
    */
class softmotor : public motor
{
    friend class chassis; //声明底盘类型为电机的友元类，让底盘能够�?�问电机的�?�有成员
public:
    int32_t Soft_RealPosition = 0; //!<�??真实�??程，这里实际意义�??�??子转过的圈数
    int32_t Soft_TargetPosition;   //!<�??�??标路程，实际意义为轮子�?�转的圈�??
    float SoftAngle;               //!<�??角度，根�??�??过圈数来进�?��?�算 RealAngle现在�??单圈内�?�度�??
    ...
    void Angle_Set(float);                                                        //!<设置�??程目标�?�度
    virtual int8_t Enable_Block(uint16_t Limit, uint16_t time, uint16_t err_num); //!<到时候写�?? 注意负数 �??坑了
protected:
    virtual void update(uint8_t Data[]);
    virtual void Position_Run(void);
    float max = 99999999999;  //!<角度最大�?
    float min = -99999999999; //!<角度最小�?
private:
    uint8_t running_flag = 0; //!<用来保证�??一次的�??�??程不突变 因为开机时LastPosition�??定是0�??能会导致一圈�?�环
};
...
public:
    ...
    float RealAngle;    //!<根据机�?��?��?�算出的真实角度
    //电流�??
    int16_t TargetCurrent; //!<发送给电机的电流�?
    int16_t RealCurrent;   //!<实际�??矩电�??(编码�??)
    //速度�??
    float RealSpeed;        //!<实际速度(陀螺仪/编码�??)
    float TargetSpeed;      //!<�??标速度
    float *Gyro_RealSpeed;  //!<指向陀螺仪反�?�速度的指�??
    float Gyro_TargetSpeed; //!<陀螺仪设置�??标转�??
    //位置�??
    int16_t RealPosition;     //!<真实位置(编码�??)
    int16_t OriginalPosition; //!<原�?�位�??(编码�??)，没有经过校�??
    int16_t TargetPosition;   //!<�??标位�??
    float *Gyro_RealAngle;    //!<指向陀螺仪反�?��?�度的指�??
    float Gyro_TargetAngle;   //!<陀螺仪设置�??标�?�度
    ...
    void Pid_Select(pid *PID_In_Select, pid *PID_Out_Select);                //!<机�?��?�pid选择
    void Gyro_Pid_Select(pid *Gyro_PID_In_Select, pid *Gyro_PID_Out_Select); //!<陀螺仪pid选择
    void Speed_Set(float);                                                   //!<设定速度，通过机�?��?�调�??
    void Angle_Set(float);                                                   //!<设定角度，通过机�?��?�调�??
...
protected:
    ...
    int16_t LastTorque;                  //!<上�?�转�??
    float LastSpeed;                     //!<上�?�速度(机�?��??)
    int16_t LastPosition;                //!<上�?�位�??(机�?��??)
    float Gyro_LastSpeed;                //!<上�?�速度(陀螺仪)
    float Gyro_LastPosition;             //!<上�?�位�??(陀螺仪)
    float max = 99999999999;             //!<角度最大�?
    float min = -99999999999;            //!<角度最小�?
    virtual void update(uint8_t Data[]); //!<直接Data数组的update函数
    virtual void Handle(void);           //!<数据处理函数，用于判�??状态，运�?�PID
...
* @brief  底盘的控制类型，带功率控�??
* @par 日志 
*   2019�??3�??9�??16:43:30 WMD 因为需要在速度�??和电流环之间插入功率限制调整函数，经过考虑重新�??改了一下�?�架，将电流�??嵌入底盘类型，而普通电机类型不涉及电流�??
*/
class chassis
{
public:
    float extra_power = 0; //利用缓冲能量计算出的额�?�功�??
    pid *Pid_extra_power;  //利用缓冲能量计算额�?�功率的pid的指�??

    static chassis *point; //!<指向当前已声明的底盘，一�??工程�??允�?�一�??底盘的存�??  该指针用于日后托管�?�理
    //pid *Turn_PID;  //!<�??弯PID
    ...
    void Handle(void);                    //!<交由CANSend托�?�的底盘处理函数
private:
    RunState_t RunState;                            //!<当前底盘状�?
